...
(defun irony-handle-syntax-check (data)
  (let ((diagnostics (plist-get data :diagnostics)))
    (message diagnostics)
))
...
(defun irony-syntax-check (&optional buffer)
  ...
  (let ((request-data (list (cons :file (irony-temp-filename))
                            (cons :flags (irony-get-libclang-flags)))))
    (irony-send-request :syntax-check request-data
                        (or buffer (current-buffer)))))
...
(defun irony-flycheck-command ()
  (concat (json-encode (list :request :syntax-check
                             :buffer 'source
                             :data (list (cons :file 'source)
                                         (cons :flags (irony-get-libclang-flags)))))
          irony-server-eot)
)

(flycheck-define-checker irony
  "A c/c++/objc syntax checker using the irony-mode server."
  :command ("/Users/arthurevstifeev/.emacs.d/personal/irony-mode/bin/irony-server"
            (eval (concat "<<EOF\n" (irony-flycheck-command) "EOF")))
  :error-parser
  (lambda (output checker buffer)
    message(output))
  :predicate
  (lambda ()
    (member major-mode irony-known-modes)))
